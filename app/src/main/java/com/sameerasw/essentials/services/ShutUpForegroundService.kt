 ...
 * File: ShutUpForegroundService.kt
 * Description: Background service component for Shut-Up! foreground app monitoring and enforcement.
 */
...
class ShutUpForegroundService : Service() {
    ...
    private val prefsChangeListener =
        SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            if (key == SettingsRepository.KEY_SHUT_UP_SERVICE_ENABLED) {
                if (settingsRepository.isShutUpServiceEnabled()) {
                    startMonitoring()
                } else {
                    monitorJob?.cancel()
                    monitorJob = null
    ...
    private var activeTargetConfig: ShutUpAppConfig? = null
    ...
    private fun startMonitoring() {
        if (!settingsRepository.isShutUpServiceEnabled()) return
        if (monitorJob != null) return
        monitorJob = serviceScope.launch {
            while (isActive) {
                val now = System.currentTimeMillis()
                // Query sliding 1000ms window to catch system_server ACTIVITY_RESUMED events instantly at cold start
                val start = (now - 1000L).coerceAtLeast(0)
                val currentPkg = getForegroundPackage(start, now)
                if (currentPkg != null) {
                    if (currentPkg != lastPackageName) {
                        ...
                        if (enforceTickCount % 5 == 0) {
                            activeTargetConfig?.let { config ->
                                val backup = settingsRepository.getShutUpOriginalSettings()
                                if (backup.isEmpty()) {
                                    // Backup was cleared (e.g. following screen lock / unlock cycle). Re-run full apply instead.
                                    Log.d(TAG, "Re-applying full ShutUp settings (empty backup) for ${config.packageName}")
    ...
    private suspend fun onPackageChanged(oldPkg: String?, newPkg: String?) {
        ...
        if (oldPkg != null && configs.any { it.packageName == oldPkg && it.isEnabled }) {
            ...
            if (newConfig == null) {
                ...
                val restoreMode = settingsRepository.getShutUpRestoreMode()
                if (restoreMode == "Notify") {
                    Log.d(TAG, "Restore mode is Notify, leaving restore flow to AppFlowHandler notification")
                } else {
                    pendingRestoreJob = serviceScope.launch {
                        ...
                        if (config != null && config.isEnabled && lastPackageName != oldPkg && activeTargetConfig == null) {
                            ShutUpManager.restoreOriginalSettings(this@ShutUpForegroundService, settingsRepository)
                            if (config.attemptShizukuRestart && settingsRepository.isShutUpAttemptShizukuRestartEnabled()) {
