...
class ScreenOffAccessibilityService : AccessibilityService(), SensorEventListener {
    ...
    private val preferenceChangeListener =
        android.content.SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            ...
            } else if (key == "wifi_optimization_enabled" || key == "wifi_auto_off_enabled" || key == "wifi_auto_off_timeout") {
                val prefs = getSharedPreferences("essentials_prefs", MODE_PRIVATE)
                val isWifiOptimizerEnabled = prefs.getBoolean("wifi_optimization_enabled", false)
                val isAutoOffEnabled = prefs.getBoolean("wifi_auto_off_enabled", false)
                if (!isWifiOptimizerEnabled || !isAutoOffEnabled) {
    ...
    private fun checkAndScheduleWifiAutoOff() {
        ...
        val isWifiOptimizerEnabled = prefs.getBoolean("wifi_optimization_enabled", false)
        val isAutoOffEnabled = prefs.getBoolean("wifi_auto_off_enabled", false)
        if (!isWifiOptimizerEnabled || !isAutoOffEnabled) return
