 ...
 * License: MIT License
 *
 * Feature Module: Audio & Sound Controls
 * File: ShutUpSettingsUI.kt
 * Description: Composable screen for media mute and ShutUp per-app rules.
 */

package com.sameerasw.essentials.ui.features.audio
...
fun ShutUpSettingsUI(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.shut_up_monitoring_service_header),
        ...
        RoundedCardContainer(
            ...
            FeatureCard(
                title = stringResource(R.string.shut_up_enable_service_title),
                description = stringResource(R.string.shut_up_enable_service_desc),
        ...
        Text(
            text = stringResource(R.string.shut_up_app_configs_header),
            style = MaterialTheme.typography.titleMedium,
            modifier = Modifier.padding(start = 16.dp),
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        ...
        if (isEditSheetOpen) {
            val isFrozen =
                remember(editingPackageName) {
                    FreezeManager.isAppFrozen(context, editingPackageName)
                }
...
private fun ShutUpAppItem(
    ...
    val autoFreezeText = stringResource(R.string.shut_up_auto_freeze_badge)
    val shizukuRestartText = stringResource(R.string.shut_up_shizuku_restart_badge)
    val descText =
        stringResource(R.string.shut_up_settings_configured_count, enabledCount) +
            (if (config.autoArchive) " • $autoFreezeText" else "") +
            (if (config.attemptShizukuRestart) " • $shizukuRestartText" else "")
