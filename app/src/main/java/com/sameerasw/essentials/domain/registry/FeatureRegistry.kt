        ...
        listOf(
            ...
            object : Feature(
                ...
                override val permissionKeys: List<String>
                    get() = (if (com.sameerasw.essentials.data.repository.SettingsRepository(EssentialsApp.context)
                ...
                override val permissionKeys: List<String>
                    get() =
                        if (ShellUtils.isRootEnabled(EssentialsApp.context)) {
                            listOf("ACCESSIBILITY", "ROOT")
                        } else {
                            listOf("ACCESSIBILITY", "SHIZUKU")
                        }

                override fun isEnabled(viewModel: MainViewModel) = viewModel.isWifiAutoOffEnabled.value

                override fun isToggleEnabled(
                    viewModel: MainViewModel,
                    context: Context,
                ) = viewModel.isAccessibilityEnabled.value &&
                    (viewModel.isShizukuPermissionGranted.value || ShellUtils.isRootEnabled(context))
